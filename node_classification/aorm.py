...
class smx_aorm_iterator:
    def __init__(self, A, k=2, shortest_only=False, method = 'sp_mm', cycle_check = False, sparseformat=csr_matrix):
    ...
    def __next__(self):
        if self.maxpower > 0 and self.power >= self.maxpower:
            ...
            if self.method == 'sp_mm':
              ...
              # self.temp = coo_matrix(self.R.dot(self.Rk))
            ...
            if self.shortest_only:
              ...
              # diff = self.temp.tocsr() - self.F.tocsr()
              # diff = self.temp - self.F.tocsr()
              diff = self.temp - self.F
              diff.data = np.heaviside(diff.data, 0)
              # self.temp = coo_matrix(diff)
              self.temp = diff
              # self.F = coo_matrix(self.temp.tocsr() + self.F.tocsr())
              # self.F = self.temp + self.F.tocsr()
              self.F = self.temp + self.F
              self.F.eliminate_zeros()             

            if self.cycle_check:
              # self.Rhat = coo_matrix(self.Rhat_temp)
            ...
            if self.temp.sum() < 0.5 :
              raise StopIteration
            # self.temp.eliminate_zeros()
            # self.Rk = sparseformat(self.temp)
            self.Rk = self.temp
            self.Rk.eliminate_zeros()

            # print(f'# Rk-{self.power} |E|: {self.Rk.nnz}\n{self.Rk}')
        if self.cycle_check:
...
# class smx_aorm_iterator:
#     def __init__(self, A, k=2, shortest_only=False, method = 'sp_mm', cycle_check = False):
#         self.power, self.maxpower = 1, k
#         self.n_nodes = A.shape[0]
#         self.n_edges = A.nnz
#         self.R = csr_matrix(A)
#         self.Rk = csr_matrix(A)
#         # self.temp = A.copy()
#         self.temp = csr_matrix(A)
#         if cycle_check: self.Rhat = csr_matrix(A)
#         # self.F = A.copy()
#         self.F = csr_matrix(A)
#         self.F.setdiag(1)
#         self.F.data = np.heaviside(self.F.data, 0)

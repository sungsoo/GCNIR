...
def aorm_precompute(features, dense_adj, degree):
    t = perf_counter()
    # print(f"# Adjacency matrix: {adj}")
    # print(f"# Feature matrix: {features}")
    # for i in range(degree):
    #     features = torch.spmm(adj, features)
    # k_aorm = aorm_iterator(dense_adj, k=degree)
    ...
    for k_aorm, power in tqdm(AormIterator(dense_adj, k=degree, shortest_only=True, method = 'edge'), desc='# AORM'):
        weight = 1.0/power*power
        # weight = 1.0
        # adj_mx, adj_mx_tensor = None
        # print(f'# Iteration-{power}: {k_aorm}')
        k_aorm = k_aorm * weight
        # print(k_aorm)
        normed_aorm = normalize(k_aorm, axis=1, norm='l1')
        # print(f'# Normalized matrix: {normed_aorm}')
        adj_mx = sp.csr_matrix(normed_aorm) 
        adj_mx_tensor = sparse_mx_to_torch_sparse_tensor(adj_mx)
        features = torch.spmm(adj_mx_tensor, features)
